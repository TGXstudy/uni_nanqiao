<template>
    <view class="goodCategoryWaper">
                ...
                <ul >
                    <li v-for="(_item, _index) in classList" :key="_index" @tap="categoryLevel(_item)" :class="'col-icon-'+val.cols">
                        <view class="lazy-img" v-if="val.iconShow" :style="{height: val.txtShow?'auto':''}">
							<image :src="$mUtils.imgUrl()+_item.gc_img"></image>
                        </view>
        ...
        <view v-else  class="no_data">
            <image src="@/static/imgs/pic_goods.png"></image>
            <view>暂无相关分类</view>
        </view>
    </view>
</template>
...
export default {
    ...
    methods:{
        ...
        pageGoodsList(gc_level, gc_id, gc_name){
            ...
            localStorage.removeItem('price_between')
            localStorage.removeItem('goods_label')
            localStorage.setItem('gc_id_type', gc_level)
            localStorage.setItem('gc_id_value', gc_id)
            localStorage.setItem('search_gc_name', gc_name)
            this.$store.dispatch('setSearchHistory',gc_name)
